...
/* Main and Proc_0 in the Ada version             */
{
    ...
    printf("Ptr_Glob->\n");
    printf("  Ptr_Comp:          0x%zx\n", (size_t)Ptr_Glob->Ptr_Comp);
...
#define NO_FLOATS
#ifdef NO_FLOATS
        int Microseconds = User_Time * Mic_secs_Per_Second / Number_Of_Runs;
        int Dhrystones_Per_Second = Number_Of_Runs / User_Time;
#else
#ifdef TIME
        float Microseconds =
            (float)User_Time * Mic_secs_Per_Second / (float)Number_Of_Runs;
        float Dhrystones_Per_Second = (float)Number_Of_Runs / (float)User_Time;
#else
        float Microseconds = (float)User_Time * Mic_secs_Per_Second /
