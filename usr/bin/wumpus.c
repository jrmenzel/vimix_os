//
// wumpus from UNIX Version 7 with minimal changes to port to modern C and
// VIMIX.
//
...
int main(int argc, char *argv[])
{
    ...
    if (rline() == 'y')
        for (i = 0; intro[i]; i++) printf(intro[i], i & 1 ? NROOM : NTUNN);
...
loop:
    ...
    for (i = 0; i < NTUNN; i++)
        if (near(&room[p->tunn[i]], WUMP)) goto nearwump;
    if (near(p, WUMP))
    {
    nearwump:
    ...
    for (i = 0; i < NTUNN; i++) printf(" %d", p->tunn[i] + 1);
    printf("\n");
...
again:
    ...
    switch (rline())
    {
        case 'm':
            ...
            i = rin() - 1;
            for (j = 0; j < NTUNN; j++)
                if (i == p->tunn[j]) goto groom;
            printf("You hit the wall\n");
            goto again;
        groom:
            loc = i;
            if (i == wloc) goto mwump;
            goto loop;
...
int near(struct room *ap, int ahaz)
{
    ...
    haz = ahaz;
    for (i = 0; i < NTUNN; i++)
        if (room[p->tunn[i]].flag & haz) return (1);
    return (0);
}
