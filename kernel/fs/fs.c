...
void inode_init(struct inode *ip, struct super_block *sb, ino_t inum)
{
    ...
    sleep_lock_init(&ip->write_exclusive_lock, "inode ex-w lock");
...
void inode_lock_2(struct inode *ip0, struct inode *ip1)
{
    DEBUG_EXTRA_PANIC(ip0 != NULL && ip1 != NULL,
                      "inode_lock_2: one of the inodes is NULL");

    sleep_lock_2(&ip0->lock, &ip1->lock);
}
...
{
    sleep_lock(&ip->write_exclusive_lock);
}

void inode_unlock_exclusive(struct inode *ip)
{
    sleep_unlock(&ip->write_exclusive_lock);
}

void inode_lock_exclusive_2(struct inode *ip1, struct inode *ip2)
{
    sleep_lock_2(&ip1->write_exclusive_lock, &ip2->write_exclusive_lock);
}
...
void debug_print_inode(struct inode *ip)
{
    ...
    printk("inode %3ld on (%2d,%2d), ", ip->inum, MAJOR(ip->i_sb->dev),
           MINOR(ip->i_sb->dev));
    ...
    {
        printk("char dev  (%2d,%2d)", MAJOR(ip->dev), MINOR(ip->dev));
    }
    else if (S_ISBLK(ip->i_mode))
    {
        printk("block dev (%2d,%2d)", MAJOR(ip->dev), MINOR(ip->dev));
    }
