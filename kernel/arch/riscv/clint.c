...
void clint_init_timer_interrupt()
{
    ...
    size_t interval = 1000000;  // cycles; about 1/10th second in qemu.
    *(uint64_t *)CLINT_MTIMECMP(id) = *(uint64_t *)CLINT_MTIME + interval;
    ...
    // interrupts.
    size_t *this_harts_scratch = &g_m_mode_interrupt_handler_scratchpads[id][0];
    this_harts_scratch[3] = CLINT_MTIMECMP(id);
    this_harts_scratch[4] = interval;
    w_mscratch((xlen_t)this_harts_scratch);

    // set the machine-mode trap handler.
    w_mtvec((xlen_t)m_mode_trap_vector);
