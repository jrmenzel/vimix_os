...
uint64 g_m_mode_interrupt_handler_scratchpads[MAX_CPUS][5];
...
void clint_init_timer_interrupt()
{
    ...
    *(uint64 *)CLINT_MTIMECMP(id) = *(uint64 *)CLINT_MTIME + interval;
    ...
    // interrupts.
    uint64 *this_harts_scratch = &g_m_mode_interrupt_handler_scratchpads[id][0];
    this_harts_scratch[3] = CLINT_MTIMECMP(id);
    this_harts_scratch[4] = interval;
    w_mscratch((uint64)this_harts_scratch);

    // set the machine-mode trap handler.
    w_mtvec((uint64)m_mode_trap_vector);
