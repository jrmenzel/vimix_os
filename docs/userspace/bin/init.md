...
First it opens the `console` character device as [/dev/console0](../dev/console.md) so that [file descriptor](../../kernel/file_system/file.md) `0`,`1` and `2` are set to `console`. These file descriptors are defined to map to `stdin`, `stdout` and `stderr` in C (see [stdio](../../misc/stdio.md)).
...
Note: **init is not expected to return**.
